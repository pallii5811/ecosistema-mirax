...
r=sb.table('searches').select('*').eq('id','c1c04ffb-bf73-4c1a-9031-de352481e54f').single().execute().data
arr=r.get('results') or []
if isinstance(arr,str):
    try: arr=json.loads(arr)
    except Exception: arr=[]
if not isinstance(arr,list): arr=[]
print(json.dumps({'id':r.get('id'),'status':r.get('status'),'created_at':r.get('created_at'),'updated_at':r.get('updated_at'),'category':r.get('category'),'location':r.get('location'),'count':len(arr),'organic':sum(1 for x in arr if is_org(x)),'maps':sum(1 for x in arr if not is_org(x)),'phones':sum(1 for x in arr if has_phone(x)),'emails':sum(1 for x in arr if has_email(x)),'error':r.get('error') or r.get('error_message')}, ensure_ascii=False, default=str))
for x in arr:
    print(('ORG' if is_org(x) else 'MAPS'), '| phone=', has_phone(x), '| email=', has_email(x), '| name=', x.get('azienda') or x.get('business_name') or x.get('nome'), '| tel=', x.get('telefono') or x.get('phone'), '| mail=', x.get('email'), '| site=', x.get('sito') or x.get('website'))
