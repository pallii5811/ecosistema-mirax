...
export default function DashboardShell() {
  ...
  const processMainSearchResults = useCallback(
    async (update: SearchRealtimeUpdate) => {
      if (searchState === 'done' || searchState === 'idle') return
      const status = update.status

      if (status === 'error' || status === 'cancelled') {
        setSearchState('done')
        setError(status === 'error' ? 'La ricerca si è conclusa con un errore. Nessun credito è stato addebitato per i risultati mancanti.' : 'Ricerca annullata.')
        return
      }

      if (status !== 'completed') return

      let nextResults = update.results
    ...
    [activeFilters, query, maxLeads, refineLeadsBySubtype, searchState],
  )

  useSearchRealtime(currentJobId, {
    onUpdate: processMainSearchResults,
    onError: processMainSearchResults,
  })

  // Refs for stable access inside the realtime callback without re-binding every render.
  const refineLeadsBySubtypeRef = useRef(refineLeadsBySubtype)
  refineLeadsBySubtypeRef.current = refineLeadsBySubtype
  const applySearchResultsRef = useRef(applySearchResults)
  applySearchResultsRef.current = applySearchResults
  const deductCreditsRef = useRef(deductCredits)
  ...
  // Realtime incremental scrape updates (replaces aggressive 3s polling).
  const processIncrementalScrapeResults = useCallback(
    async (update: SearchRealtimeUpdate) => {
      if (!isScraping) return
      const status = update.status
      ...
      const expectedQuery = activeSearchQueryRef.current
      const _tf2 = buildTechFilter(expectedQuery || queryRef.current)
      const activeFiltersValue = activeFiltersRef.current
      const applyAllFilters = (leads: any[]) => {
        let out = leads
        if ((activeFiltersValue as any)?.has_website === false) {
          out = out.filter((lead: any) => {
            const s = (typeof lead?.sito === 'string' ? lead.sito : typeof lead?.website === 'string' ? lead.website : '').trim()
            return !s || s === 'N/D' || s === 'N/A' || s === 'N.D.'
          })
        } else if ((activeFiltersValue as any)?.has_website === true) {
          out = out.filter((lead: any) => {
            const s = (typeof lead?.sito === 'string' ? lead.sito : typeof lead?.website === 'string' ? lead.website : '').trim()
            return s && s !== 'N/D' && s !== 'N/A' && s !== 'N.D.'
          })
      ...
      const normalized = deduplicateResults(applyAllFilters(parsed.map(normalizeLeadFields))) as any[]
      const jobResults = normalized.filter(_hasContact)
      const refinedResults = await refineLeadsBySubtypeRef.current(jobResults)
      ...
      if (status === 'error' || status === 'cancelled') {
        ...
        setSearchState('done')
        if (capped.length > 0) {
          applySearchResultsRef.current(capped, true)
          toastError('La ricerca si è conclusa con un errore. I risultati già mostrati restano disponibili.', 'Errore ricerca')
        } else {
          setSearchError('La ricerca ha riscontrato un errore prima di produrre risultati. Nessun credito è stato addebitato.')
        }
        return
      }

      // Partial updates while processing/pending.
      if (status !== 'completed') {
        if (capped.length > 0) {
          applySearchResultsRef.current(capped)
          const previousCount = resultsCountRef.current
          const newCount = Math.max(0, capped.length - previousCount)
          if (newCount > 0) {
            await deductCreditsRef.current(newCount)
          }
        }
        return
      }
  ...
  useSearchRealtime(scrapeJobId, {
    onUpdate: processIncrementalScrapeResults,
    onDone: processIncrementalScrapeResults,
    onError: processIncrementalScrapeResults,
  })

  // Fallback guard: if incremental scrape never finishes, surface results/stall.
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      if (currentResults.length > 0) {
        ...
        setSearchStuck(true)
        setSearchError('La ricerca è in stallo: nessun risultato ricevuto entro il tempo limite. Nessun credito è stato addebitato.')
      }
    }, 10 * 60 * 1000)
  ...
  return (
    <>
      ...
      {!isLoading && results.length === 0 ? (
        searchStuck || searchError ? (
          <div className="flex flex-col items-center justify-center py-20 text-center px-4">
            <div className="rounded-2xl border border-rose-200 bg-rose-50 px-6 py-5 text-rose-900 max-w-md">
              <p className="font-semibold">{searchError ?? 'Ricerca in stallo'}</p>
              <p className="mt-2 text-sm text-rose-700">
                Puoi riprovare con una query diversa o un&apos;area più ampia. Nessun credito è stato scalato per i risultati non ricevuti.
              </p>
              <Button
                type="button"
                variant="outline"
                className="mt-4"
                onClick={() => { resetSearchRuntime(''); setQuery('') }}
              >
                Nuova ricerca
              </Button>
            </div>
        ...
        <>
          {Array.isArray(results) && results.length > 0 ? (
            <>
              ...
              {(searchStuck || searchError) && (
                <div className="flex items-center gap-2 bg-rose-50 border border-rose-200 rounded-xl px-4 py-2.5 mb-3 mx-4 text-sm text-rose-900 font-medium">
                  {searchError ?? 'Ricerca in stallo: i risultati parziali restano disponibili e non verranno addebitati ulteriori crediti.'}
                </div>
              )}

              {!isScraping && !autoScrapeLoading && searchState === 'done' && results.length > 0 && pendingAuditCount === 0 && (results.length >= maxLeads || searchExhausted) && !searchStuck && !searchError && (
