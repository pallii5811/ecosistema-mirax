...
export async function POST(req: NextRequest) {
  ...
  const integration = await loadCrmIntegration(supabase, user.id, integrationId, 'hubspot')
  ...
  if (!first || first.status === 'error') {
    return Response.json({ error: first?.error || 'HubSpot API error' }, { status: 400 })
  }

  return Response.json({ success: true, hubspot_id: first.external_id ?? null })
}
