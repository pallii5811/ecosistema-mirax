...
export async function POST(req: NextRequest) {
  ...
  const integration = await loadCrmIntegration(supabase, user.id, integrationId, 'webhook')
  ...
  if (!first || first.status === 'error') {
    return Response.json({ error: first?.error || 'Webhook failed', success: false }, { status: 500 })
  }
