...
def normalize_phone_italy(value: Optional[str]) -> Optional[str]:
    ...
    has_explicit_prefix = raw.startswith("+") or raw.startswith("00")
    digits = re.sub(r"\D+", "", raw)
    if not digits:
        return None

    # Rule: if local mobile (9/10 digits) and starts with 3 => force +39
    if not has_explicit_prefix and len(digits) in (9, 10) and digits.startswith("3"):
        return "+39" + digits

    # Otherwise: do NOT infer any country code. Preserve explicit international numbers.
    if raw.startswith("+"):
        return "+" + digits
...
_MOBILE_IT_RE = re.compile(r"\b3\d{2}\s?\d{3}\s?\d{4}\b")
...
async def deep_scrape_mobile_from_website(website: str, html_home: Optional[str] = None) -> Optional[str]:
    ...
    found = _extract_mobile_it_from_text(html_home)
    if found:
        return found
            ...
            for path in candidates:
                try:
                    ...
                    if r.status_code >= 400:
                        continue
                    found = _extract_mobile_it_from_text(r.text or "")
                    if found:
                        return found
                except Exception:
                    continue
...
app = FastAPI(title="Client Sniper & Audit Backend", version="0.1.0")
...
DEMO_ENABLED = True
DEMO_CITTA_ABILITATE = ["Milano", "Roma"]
DEMO_CATEGORIE_ABILITATE = ["Idraulico", "Elettricista", "Ristorante", "Pizzeria"]
DEMO_MAX_RESULTS = 20
DEMO_MSG_CITY = (
    "⛔ VERSIONE DEMO: Puoi cercare solo su Milano o Roma. "
    "Acquista la licenza per sbloccare l'Italia."
)
DEMO_MSG_CATEGORY = "⛔ VERSIONE DEMO: Categoria non disponibile. Acquista la licenza Lifetime."
DEMO_MSG_LIMIT = (
    "🚀 HAI RAGGIUNTO IL LIMITE DELLA DEMO (20 Contatti). "
    "Per scaricarne altri 10.000, contatta Simone per la licenza completa."
)
...
# --- DEMO DATA (static, sales-oriented) ---
DEMO_DATA: List[Dict[str, Any]] = [
    {
        "company": "Ristorante Da Vittorio (DEMO)",
    ...
    {
        "company": "Parrucchiere Style (DEMO)",
        "address": "Roma Centro",
        "phone": "+39 333 1234567",
        "website": "www.style.it",
        "has_pixel": False,
        "status": "✅ WHATSAPP",
        "employees": "1-10",
        "revenue": "Medium",
    ...
    {
        "company": "Impresa Edile Costruire",
        "address": "Milano Nord",
    ...
    },
    {
        ...
        "address": "Roma Sud",
        "phone": "+39 340 4445551",
        "website": "www.bella.com",
        "has_pixel": True,
        "status": "✅ WHATSAPP",
        "employees": "1-5",
        "revenue": "Medium",
    },
    {
        ...
        "address": "Milano",
        "phone": "+39 392 1122334",
        "website": "www.pizzanapoli.it",
        "has_pixel": False,
        "status": "✅ WHATSAPP",
        "employees": "5-15",
        "revenue": "Medium",
    ...
    with sync_playwright() as p:
        for attempt, q in enumerate(query_variants, start=1):
            ...
            try:
                ...
                cap_new = DEMO_MAX_RESULTS if DEMO_ENABLED else 25
...
async def run_job(job: Job) -> None:
    ...
    try:
        ...
        # DEMO quality filter: show mobiles first
        if DEMO_ENABLED:
            def _demo_mobile_priority(it: Dict[str, Any]) -> Tuple[int, str]:
                p = normalize_phone_italy(it.get("phone")) or ""
                pri = 0 if p.startswith("+393") else 1
                name = str(it.get("business_name") or "").lower()
                return (pri, name)

            raw = sorted(list(raw), key=_demo_mobile_priority)
            raw = list(raw)[:DEMO_MAX_RESULTS]
        ...
        for i, item in enumerate(raw):
            ...
            # DEMO hard stop at 20 extracted contacts
            if DEMO_ENABLED and len(results) >= DEMO_MAX_RESULTS:
                job.results = list(results)
                job.state = "done"
                job.finished_at = time.time()
                await job.emit(100, DEMO_MSG_LIMIT)
                return
...
def _demo_results_as_business_results() -> List[BusinessResult]:
    results: List[BusinessResult] = []
    for i, it in enumerate(DEMO_DATA):
        name = str(it.get("company") or "").strip() or "DEMO"
        address = str(it.get("address") or "").strip() or None
        phone = str(it.get("phone") or "").strip() or None
        website_raw = str(it.get("website") or "").strip()
        website = normalize_website(website_raw) if website_raw else None
        has_pixel = bool(it.get("has_pixel"))
        website_status: WebsiteStatus = "HAS_WEBSITE" if website else "MISSING_WEBSITE"
        ...
        results.append(
            BusinessResult(
                ...
                business_name=name,
                address=address,
                phone=normalize_phone_italy(phone),
                email=None,
                website=website,
                website_status=website_status,
                tech_stack=None,
                ...
                domain_creation_date=None,
                domain_expiration_date=None,
                website_http_status=200 if website else None,
                website_error=None,
                website_has_html=False,
...
async def _run_demo_job(job: Job) -> None:
    job.state = "running"
    try:
        await job.emit(10, "Avvio DEMO...")
    except Exception:
        pass
    await asyncio.sleep(1.5)
    job.results = _demo_results_as_business_results()
    job.state = "done"
    job.finished_at = time.time()
    try:
        await job.emit(100, "DEMO pronta. Risultati caricati.")
    except Exception:
        pass
...
async def start_job(
    ...
    payload: Any = Body(default=None, embed=False),
) -> Any:
    data: Dict[str, Any] = payload if isinstance(payload, dict) else {}

    # Accept anything without ever raising 400/422.
    city = str(data.get("city") or "Milano")
    category = str(data.get("category") or "Ristoranti")
    zone = str(data.get("zone") or "")

    # Force demo-safe defaults if user sends nonsense.
    if city.strip() not in DEMO_CITTA_ABILITATE:
        city = DEMO_CITTA_ABILITATE[0]
    if category.strip() not in DEMO_CATEGORIE_ABILITATE:
        category = DEMO_CATEGORIE_ABILITATE[0]

    job_id = str(uuid.uuid4())
    job = Job(id=job_id, category=category, city=city, zone=zone)
    JOBS[job_id] = job
    background.add_task(_run_demo_job, job)

    return {
        "id": job.id,
        "job_id": job.id,
        "status": "pending" if job.state in {"queued", "running"} else "finished",
        "state": job.state,
...
@app.post("/api/jobs")
async def start_job_api(
    background: BackgroundTasks,
    payload: Any = Body(default=None, embed=False),
) -> Any:
    return await start_job(background=background, payload=payload)
...
async def catch_all(full_path: str, request: Request) -> Any:
    try:
        print(f"Richiesta DEMO ricevuta su /api/{full_path}")
    except Exception:
        pass
    await asyncio.sleep(0.5)

    # Universal API fallback: always reply with a structure rich enough for picky UIs.
    if request.method.upper() in {"POST", "PUT", "DELETE", "OPTIONS"}:
        return JSONResponse(
            content={
                "id": "demo_job_123",
                "job_id": "demo_job_123",
    ...
    # GET: return demo results in multiple keys for compatibility.
    return JSONResponse(
        content={
            "id": "demo_job_123",
            "job_id": "demo_job_123",
...
@app.api_route("/{path_name:path}", methods=["POST", "PUT", "DELETE", "OPTIONS"])
async def catch_all_any(path_name: str, request: Request) -> Any:
    # IMPORTANT: do NOT catch GET here, otherwise we'd shadow the static frontend mount at '/'.
    try:
        print(f"Richiesta DEMO ricevuta su: {path_name}")
    except Exception:
        pass
...
async def get_job(job_id: str) -> Any:
    ...
    # Return a rich payload: job status + embedded results (some UIs expect result/data/items here)
    return {
        "id": job.id,
        "job_id": job.id,
        "status": "pending" if job.state in {"queued", "running"} else "finished",
        ...
        "error": job.error,
        "results_count": len(job.results),
        "result": [r.model_dump() for r in job.results],
        "data": [r.model_dump() for r in job.results],
        "items": [r.model_dump() for r in job.results],
        "completed": job.state == "done",
    }


@app.get("/api/jobs/{job_id}")
async def get_job_api(job_id: str) -> Any:
...
@app.get("/api/jobs/{job_id}/results", response_model=List[BusinessResult])
async def get_results_api(job_id: str) -> List[BusinessResult]:
...
@app.get("/jobs/{job_id}/results/{result_index}/technical-audit", response_model=TechnicalAuditResult)
async def get_technical_audit(job_id: str, result_index: int) -> TechnicalAuditResult:
    job = JOBS.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    ...
    if row.website_status != "HAS_WEBSITE" or not row.website:
        return TechnicalAuditResult(
            url=str(row.website or ""),
            final_url=str(row.website or ""),
            http_status=200,
            issues=[],
            has_critical=False,
        )
