...
print('BEFORE', json.dumps({'id':r.get('id'),'status':r.get('status'),'created_at':r.get('created_at'),'count':len(arr),'category':r.get('category'),'location':r.get('location')}, ensure_ascii=False))
now=datetime.now(timezone.utc).isoformat()
sb.table('searches').update({'status':'pending','created_at':now}).eq('id',job_id).execute()
r2=sb.table('searches').select('id,status,created_at,results').eq('id',job_id).single().execute().data
arr2=r2.get('results') or []
if isinstance(arr2,str):
    try: arr2=json.loads(arr2)
    except Exception: arr2=[]
print('AFTER', json.dumps({'id':r2.get('id'),'status':r2.get('status'),'created_at':r2.get('created_at'),'count':len(arr2)}, ensure_ascii=False))
