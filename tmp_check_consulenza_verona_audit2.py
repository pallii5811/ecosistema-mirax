...
rows=sb.table('searches').select('id,status,category,location,results,created_at,user_id').or_('category.ilike.%consulenza%,category.ilike.%aziendale%').ilike('location','%Verona%').order('created_at', desc=True).limit(10).execute().data or []
print('ROWS',len(rows))
for r in rows:
    ...
    for x in arr:
        ...
        if tr: tech+=1
        if au or tr.get('organic_audited') or tr.get('status') or tr.get('seo_errors') is not None or tr.get('load_speed_seconds') is not None: audited+=1
        else: pending+=1
    ...
    print(json.dumps({'id':r.get('id'),'status':r.get('status'),'created_at':r.get('created_at'),'category':r.get('category'),'location':r.get('location'),'count':len(arr),'tech':tech,'audited':audited,'pending_like':pending,'organic':organic}, ensure_ascii=False))
