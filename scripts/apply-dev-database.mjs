#!/usr/bin/env node
/**
 * Applica schema + migration su Supabase DEV (Node pg client).
 */
import fs from 'fs'
...
function loadPassword() {
  const sources = [ENV_PATH, SECRETS_PATH]
...
try {
  ...
  console.log('\nDone. Verifica tabelle in Supabase → Table Editor.')
  await client.end()
} catch (e) {
  console.error('Errore apply DB:', e.message)
  process.exit(1)
}
