...
async function runViaRest(sb) {
  ...
  // I cataloghi di sistema (information_schema, pg_indexes, pg_proc) non sono esposti
  // via PostgREST come tabelle utente; usiamo sempre Postgres diretto per i dettagli schema.
  console.log('  (dettagli colonne/indici/funzioni via Postgres diretto)')
}
...
try {
  ...
  if (restTableCheck) {
    try {
      await runViaPg()
      console.log('\n[test-universe-schema] OK (REST + Postgres)')
    } catch (pgErr) {
      const pgMsg = pgErr instanceof Error ? pgErr.message : String(pgErr)
      if (isNetworkError(pgMsg)) {
        console.warn('Postgres diretto non raggiungibile — dettagli schema verificati solo via REST:', pgMsg)
