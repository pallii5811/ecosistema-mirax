-- Clamp settle so committed_cost_eur never exceeds hard_cost_eur.
-- Prevents S1-class overshoot (€0.0656 > €0.05) when actual > estimate.
...
begin
  ...
  v_clamped_actual := least(p_actual_cost_eur, v_remaining);
  if p_actual_cost_eur > v_remaining then
    v_overshot := true;
  end if;
  v_new_committed := v_without_current + v_clamped_actual;

  update public.search_cost_ledger set
    actual_cost_eur = v_clamped_actual,
    status = 'settled',
    metadata = metadata || coalesce(p_metadata, '{}'::jsonb) || jsonb_build_object(
      'partial_budget_exhausted', v_overshot,
      'requested_actual_cost_eur', p_actual_cost_eur,
      'clamped_actual_cost_eur', v_clamped_actual
    ),
